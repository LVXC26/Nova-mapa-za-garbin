...
import { MapPin, Calendar, Ruler, BedDouble, Users, Heart, GitCompare } from 'lucide-react'
  ...
  return (
    <Link href={`/plovila/${plovilo.id}`} className="group block">
      <div className={`bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 ${
        jePremium ? 'border-2 border-[#c9a84c]' : 'border border-gray-100'
      }`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {plovilo.max_oseb && (
              <div className="flex items-center gap-1.5 text-xs text-gray-500">
                ...
                <span>Maks. {plovilo.max_oseb} oseb</span>
              </div>
            )}
